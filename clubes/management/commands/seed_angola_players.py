...
class Command(BaseCommand):
    help = 'Gera jogadores de Angola para um clube ou todos os clubes de um tenant'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--list-clubs', action='store_true', help='Lista clubes do tenant e termina')
    ...
    def handle(self, *args, **options):
        ...
        if list_clubs:
            self.stdout.write(
                self.style.MIGRATE_HEADING(f'Clubes do tenant "{tenant.slug}":')
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f'{clubs_qs.count()} clubes, {total_created} jogadores criados, {total_updated} atualizados'
            )
        )
